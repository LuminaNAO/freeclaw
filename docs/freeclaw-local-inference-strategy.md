...
- **4 slots (default)**: Only slot 3 was used throughout all tasks so far. Idle slots consume KV cache VRAM. However, openclaw spawns sub-agents which each make independent inference calls — reducing to 1 slot risks queuing deadlocks if a main agent waits on a sub-agent that is itself queued behind it. **Recommendation: `--parallel 2`** — covers main + 1 concurrent sub-agent, halves idle KV waste vs 4 slots. Do not go to 1. Note: VRAM is pre-allocated at launch so this only helps on next server restart.
