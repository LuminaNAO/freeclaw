...
import {
  loadGatewayTlsRuntimeMock as loadGatewayTlsRuntime,
...
describe("resolveGatewayConnection", () => {
  ...
  it("fails when both local token and password are configured but gateway.auth.mode is unset", async () => {
    ...
    await expect(resolveGatewayConnection({})).rejects.toThrow(
      "gateway.auth.mode is unset. Set gateway.auth.mode to token, password, or token-password.",
    );
  });

  it("uses both local token and password when gateway.auth.mode is token-password", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("pins local autogenerated TLS for wss TUI connections", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        port: 40741,
    ...
    const result = await resolveGatewayConnection({});
    expect(result.url).toBe("wss://127.0.0.1:40741");
    expect(result.tlsFingerprint).toBe("AA:BB:CC");
  });
